...
**Última etapa concluída:** transações `TwDoc`/`applyOperation`, núcleo Yjs Dart local, cursor geometry-free, base de layout text (incluindo UAX #14 básico), serialização JSON/HTML validada e o primeiro reducer `EditorState` com inserção, seleção, delete e undo/redo. Tudo está coberto pela suíte Dart em `test/`; a implementação completa ainda permanece pendente.
