...
**Última etapa concluída:** transações `TwDoc`/`applyOperation`, núcleo Yjs Dart local, cursor geometry-free, base de layout text (UAX #14 e UAX #9 em baseline), serialização JSON/HTML validada e o primeiro reducer `EditorState` com inserção, seleção, delete e undo/redo. Tudo está coberto pela suíte Dart em `test/`; tabelas Unicode completas, editor integral, backends e colaboração ainda permanecem pendentes.
