...
**Última etapa concluída:** transações `TwDoc`/`applyOperation`, núcleo Yjs Dart local, cursor geometry-free, base de layout text (UAX #14 e UAX #9 em baseline), serialização JSON/HTML validada, reducer `EditorState` inicial e modelo geométrico base do print (`LayoutBox`/`PageBox`). Tudo está coberto pela suíte Dart em `test/`; tabelas Unicode completas, editor integral, BFC/IFC, backends e colaboração ainda permanecem pendentes.
