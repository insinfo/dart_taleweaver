...
OperationResult applyOperation(
  ...
  // Collect dirty ids from the doc's last transaction.
  // We wire this through the afterTransaction callback.
  // For simplicity, we just invalidate the entire cache and
  // return a fresh state.

  // Invalidate the snapshot cache.
  state.snapshotCache.invalidateAll();

  // Create a fresh state sharing the same doc but with a clean cache.
  final newState = freshState(state);

  return OperationResult(state: newState, dirtyIds: dirtyIds);
}
