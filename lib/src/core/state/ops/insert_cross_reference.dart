...
OperationResult insertCrossReference(
  ...
  final plan = _planCrossReferenceInsert(
      state, position, targetId, refMode, numberStyle);
...
_CrossReferenceInsertPlan _planCrossReferenceInsert(
  ...
  if (resolved == null) {
    throw StateError(
        'insertCrossReference: block "${position.blockId}" not found');
  }
  final block = resolved.block;
  final kind = resolved.kind;
  if (block.inlineContent == null) {
    throw StateError(
        'insertCrossReference: block "${position.blockId}" is not a leaf');
  }
