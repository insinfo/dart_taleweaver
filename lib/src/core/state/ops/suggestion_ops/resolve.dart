...
_BlockScanResult _resolveBlockScan(
  ...
  for (final block in iterateBlocksInDocumentOrder(state)) {
    ...
    for (final item in content.items) {
      ...
      if (r is! ScanKeep) {
        ...
        if (r is ScanBreakDrop && r.merge) {
          final kind =
              resolveBlock(state, block.id)?.kind ?? ResolvedBlockKind.main;
    ...
    if (touched) {
      final kind =
          resolveBlock(state, block.id)?.kind ?? ResolvedBlockKind.main;
      writes.add(ResolveDecisionWrite(
          blockId: block.id, kind: kind, decisions: decisions));
    }
  }
  ...
  return applyOperation(state, (d) {
    for (final write in scan.writes) {
      applyResolveDecisionsInTx(
          d, write.blockId, write.kind, write.decisions, customEquals);
    }
...
OperationResult acceptSuggestion(State state, SuggestionId id,
    [Map<String, AttrEqualsFn>? customEquals]) {
  return _resolve(state, id, true, customEquals);
}

OperationResult rejectSuggestion(State state, SuggestionId id,
    [Map<String, AttrEqualsFn>? customEquals]) {
...
OperationResult _resolve(
  ...
  ScanItemResult classify(InlineItem item) {
    if (_isBreakEmbed(item) &&
        (item as EmbedItem).properties['suggestionId'] == id.value) {
    ...
    if (item is TextItem && item.attrs[attrKey] == id.value) {
      switch (action) {
        ...
        case ResolveAction.applyStrip:
          return ScanRewrite(
              mergeAttrs(_attrsWithout(item.attrs, attrKey), proposedAttrs));
      }
    }
...
String _attrKeyByKind(String kind) {
  switch (kind) {
    case 'insertion':
      return insertionSuggestionAttr;
    case 'deletion':
      return deletionSuggestionAttr;
    case 'formatting':
      return formattingSuggestionAttr;
    default:
      return '';
  }
}
...
EmbedItem? _resolveEmbedFormatting(EmbedItem item, bool isAccept, TwDoc doc,
    [SuggestionId? onlyId]) {
  ...
  return EmbedItem(
      embedType: item.embedType, attrs: attrs, properties: item.properties);
}

OperationResult acceptAll(State state,
    [Map<String, AttrEqualsFn>? customEquals]) {
  return _resolveAll(state, true, customEquals);
}

OperationResult rejectAll(State state,
    [Map<String, AttrEqualsFn>? customEquals]) {
  return _resolveAll(state, false, customEquals);
}

OperationResult _resolveAll(
    State state, bool isAccept, Map<String, AttrEqualsFn>? customEquals) {
  final doc = state.doc;
...
_AllRewrite _acceptAllRun(TextItem item, TwDoc doc) {
  ...
  if (hasFormatting) {
    ...
    attrs =
        mergeAttrs(_attrsWithout(attrs, formattingSuggestionAttr), proposed);
  }
  return _AllRewrite(
      touched: true, keep: true, item: TextItem(text: item.text, attrs: attrs));
}
...
void mergeWithNextSiblingLiveInTx(
    TwDoc doc, BlockId blockId, ResolvedBlockKind kind,
    [Map<String, AttrEqualsFn>? customEquals]) {
